...
在 Codex 设置里开关 MCP，只影响工具调用。图片自动拦截由 Codex `base_url` 是否指向 CodexLens 代理决定，因此需要单独开关。
...
大多数用户不需要自己运行命令。直接对 Codex 说“请开启 CodexLens 的图片自动拦截”或“请关闭 CodexLens 的图片自动拦截，但保留 MCP 工具”即可。
