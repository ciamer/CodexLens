...
当你把图片粘贴到 Codex 输入框时，CodexLens 会在图片到达下游模型之前先截住它，把图片发给 Qwen 等视觉模型，得到文字说明后，再把这段说明连同你的问题交给下游模型。
...
Word、PDF 和手动图片分析由 CodexLens MCP 服务器提供。若你不再需要这些工具，可以在 Codex 的 **设置** 中找到 **MCP 服务器**，将 **CodexLens** 关闭。
