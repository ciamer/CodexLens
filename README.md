...
图片自动拦截默认开启。**只有在 Codex 设置中启用了 CodexLens MCP 时，才能通过对话开关图片自动拦截。** 如果你已在设置中关闭了 CodexLens，请先重新启用它并重启 Codex。

满足这个前提后，直接对 Codex 说下面任意一句：
...
在已启用 CodexLens MCP 的会话中，Codex 会调用 CodexLens 的开关工具。开启时会修改 Codex 的 `base_url`，安装脚本会先备份原配置；关闭时会恢复它。每次开关后都要重启 Codex。若整个 CodexLens MCP 已在设置中关闭，请先在设置中启用它。具体说明见 [图片自动拦截开关](docs/image-proxy-toggle.md)。
...
CodexLens 提供图片/文档分析工具，以及 `get_image_interception_status`、`enable_image_interception` 和 `disable_image_interception` 三个图片自动拦截管理工具。项目目录、手动运行方式、测试命令和已知限制请查看 [完整技术说明](docs/python-environment.md)、[图片自动拦截开关](docs/image-proxy-toggle.md) 和源代码注释。
